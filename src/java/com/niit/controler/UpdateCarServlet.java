...
public class UpdateCarServlet extends HttpServlet {
    ...
    System.out.println("carMake "+carMake+" carModel "+carModel+" varient "+variant+" mfgYear "+mfgYear+
     ...
     CarInfo car = new CarInfo( carMake, carModel, variant, mfgYear, carFuel, owners, price, kms, mileage,branch);
         boolean carId=DAOImp.updateCar(Integer.parseInt(request.getParameter("carId")),car) ;
                 if(carId==true){
                    System.out.println("User Record Successfully Updeted");
                    request.setAttribute("carId",carId );
                    RequestDispatcher rd = request.getRequestDispatcher("ModifyPage.jsp");
                    rd.forward(request, response);
                }
            else
                {
                    System.out.println("Failed to Update Car Record!");
                    out.println("Failed to Update Car ");
